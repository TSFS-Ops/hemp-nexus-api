...
## D. Operator hand-off (for james@izenzo.co.za)

Plain steps for a non-technical person:

1. Open the app at `https://trade.izenzo.co.za` and sign in as
   **james@izenzo.co.za**.
...
3. Scroll to the bottom of the Billing page. You will see an
   ...
   (If you do not see it, you are not signed in as a platform admin —
   stop and tell us.)
4. Click **"Start PayFast Sandbox Test"**. A new browser tab opens
   on the PayFast sandbox payment page
...
6. After payment, PayFast will redirect you back to
   ...
   land on the Billing page with a `?payfast=return` URL.
7. Send this message back: **"Sandbox payment completed at <time>,
   reached the return page."** (One sentence is fine. Do not paste any
   card details or any URL fragments after a `#`.)

After you send that, the team will:
...
Current status:
`PAYFAST_PHASE_2F_SANDBOX_ROUND_TRIP_UNBLOCKED_AWAITING_HUMAN_RUN`
