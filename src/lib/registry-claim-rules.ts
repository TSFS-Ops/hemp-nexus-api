/**
 * Batch 7 — Registry Search / Claim Rules Hardening SSOT (browser mirror).
 *
 * Mirror: supabase/functions/_shared/registry-claim-rules.ts
 * Pinned by:
 ...
 *
 * Encodes the approved operating rules for registry search, public profile
 * display, claim eligibility, new-company requests, claim conflict handling,
 * correction requests, field visibility, outreach blocking and conservative
 * readiness defaults. See:
 *   evidence/batch-7-registry-search-claim-rules-hardening/README.md
 *
 * NEVER hand-edit copy strings in components — import from here.
 */

/** Allowed claimant role types (Q1). */
...
/** Professional representative roles cannot edit profile / submit bank
 *  details / approve API sharing / manage users without an approved
 *  authority-to-act. Default authority period is 90 days. */
...
/** Roles allowed to approve claims (Q1). */
export const REGISTRY_CLAIM_APPROVAL_ROLES = [
  "platform_admin",
  "compliance_owner",
] as const;
export type RegistryClaimApprovalRole =
  (typeof REGISTRY_CLAIM_APPROVAL_ROLES)[number];

/** Q2 — claim interest / pre-account journey states. */
...
/** Q6 — competing claim conflict states. */
...
/** Q4 — evidence requirement categories by entity / claimant type. */
...
export type RegistrySearchabilityTier =
  (typeof REGISTRY_SEARCHABILITY_TIERS)[number];

export const REGISTRY_SEARCH_FIELD_TIERS: Record<
  RegistrySearchabilityTier,
  readonly string[]
> = {
  public_searchable: [
    "company_name",
...
/** Q8 — public profile visibility tiers. */
...
export type RegistryVisibilityTier =
  (typeof REGISTRY_VISIBILITY_TIERS)[number];

export const REGISTRY_PROFILE_FIELD_TIERS: Record<
  RegistryVisibilityTier,
  readonly string[]
> = {
  public_visible: [
    "company_name",
...
/** Q9 — imported record readiness states (conservative defaults). */
...
/** Q10 — new-company request states. */
...
/** Q12 — outreach channel permission states. */
...
export type RegistryOutreachChannelPermission =
  (typeof REGISTRY_OUTREACH_CHANNEL_PERMISSIONS)[number];

/** Audit event names added by Batch 7. */
...
export type RegistryBatch7AuditEventName =
  (typeof REGISTRY_BATCH7_AUDIT_EVENT_NAMES)[number];

/**
 * Canonical safety copy. Pinned verbatim by
 * scripts/check-registry-claim-rules-forbidden-wording.mjs.
 */
...
/** Allowed API status responses for imported_unverified records (Q9). */
...
/** Search features required by Q7. */
...
/** Minimum required fields for a new-company request (Q10). */
...
export function isProfessionalRepresentative(
  role: RegistryClaimantRoleType,
): boolean {
  return REGISTRY_PROFESSIONAL_REPRESENTATIVE_ROLES.includes(role);
}

export function evidenceRequirementsFor(
  category: RegistryEvidenceCategory,
): readonly string[] {
  return REGISTRY_EVIDENCE_REQUIREMENTS[category];
}

export function isImportedUnverified(
  state: RegistryImportedRecordReadinessState,
): boolean {
  return state === REGISTRY_IMPORTED_RECORD_DEFAULT_STATE;
}
