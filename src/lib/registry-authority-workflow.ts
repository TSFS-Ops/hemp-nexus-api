/**
 * Batch 12 — Authority-to-Act Request, Evidence and Approval SSOT (browser).
 *
 * Mirror: supabase/functions/_shared/registry-authority-workflow.ts
 * Pinned by:
 *   - scripts/check-registry-authority-workflow-parity.mjs
 *   - scripts/check-registry-batch12-authority-audit-names.mjs
 *   - scripts/check-registry-batch12-authority-wording.mjs
 *
 * NON-NEGOTIABLE:
 *  - Authority approval grants ONLY the named scopes.
 ...
 *  - In-app notifications are LOG-ONLY in this batch.
 *  - Raw sensitive evidence must never be public.
 */
...
/** Mandatory acknowledgement copy shown on admin authority approval. */
...
/** Mandatory public-safe wording on approved authority surfaces. */
...
/** Public-safe wording on rejection. */
...
function reqsForScope(
  ...
  if (
    ["disabled", "archived"].includes(input.companyLifecycleState)
  ) {
    blockers.push("company_not_actionable");
  }
  if (!input.approvedClaimType) {
    blockers.push("approved_claim_required");
  }
...
// ---------------------------------------------------------------------------
// Overall status reducer for scope decisions
// ---------------------------------------------------------------------------
