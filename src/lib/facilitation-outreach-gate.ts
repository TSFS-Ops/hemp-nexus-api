 ...
 * status and any open compliance escalation, and returns a single
 * `allow | warn | block` decision plus a list of reason codes.
 *
 * It is intentionally I/O-free: the caller (server edge function or
 ...
 * credit/payment/engagement records.
 */
...
export interface DncRuleSnapshot {
  ...
  /** Lower-cased canonical value: email, domain, or org name. */
  value: string;
  /** Only "active" rules contribute; "revoked" rules are ignored by the caller. */
...
export interface OutreachGateInput {
  ...
  /** Caller looks this up via the existing suppressed_emails table. */
  suppression_active: boolean;
  /** Any row in facilitation_compliance_escalations with status='open' for this case. */
