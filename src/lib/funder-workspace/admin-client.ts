...
// Batch 12 / Phase 2: optional supersede request. When a current sealed
// pack already exists for the release, the edge function/RPC will reject
// a plain (non-supersede) call with a 409 supersede_required error — the
// caller must re-invoke with { supersede: true, supersedeReason } and a
// reason of at least 10 characters (enforced both client- and server-side).
export async function generateSealedPack(
    releaseId: string,
    opts?: { supersede?: boolean; supersedeReason?: string },
  ): Promise<GenerateSealedPackResult> {
    const body: Record<string, unknown> = { release_id: releaseId };
    if (opts?.supersede) {
          body.supersede = true;
          body.supersede_reason = opts.supersedeReason;
    }
