/**
 * Batch 11 — Real Claim Submission, Evidence Upload and Review UX SSOT (browser).
 *
 * Mirror: supabase/functions/_shared/registry-claim-workflow.ts
 ...
 *
 * NON-NEGOTIABLE:
 *  - Claim approval never grants authority, never verifies the company profile,
 *    never verifies bank details, and never enables API/institutional use.
 *  - In-app notifications are LOG-ONLY in this batch.
 *  - Sensitive evidence is never public.
 */
...
/** Default claim expiry rules. Days. */
...
/** Mandatory public wording. Pinned verbatim. */
...
/** Evidence-requirements engine input. */
...
export function evaluateClaimEvidenceRequirements(
  ...
  // Professional reps always need an identity proof + mandate
...
/** Professional reps may only do these things post-approval. */
export const REGISTRY_PROFESSIONAL_REP_ALLOWED_ACTIONS = [
  "start_claim",
  "upload_evidence",
  "request_authority_review_later",
] as const;

export const REGISTRY_PROFESSIONAL_REP_BLOCKED_ACTIONS = [
  "edit_company_profile_directly",
  "submit_bank_details",
  "approve_api_sharing",
