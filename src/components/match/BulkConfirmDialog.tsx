...
export function BulkConfirmDialog({
  ...
  const {
    ...
    queryKey: ["token-balance-confirm", userProfile?.org_id],
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("token_balances").select("balance").eq("org_id", userProfile!.org_id).single();
      if (error) throw error;
      return data?.balance ?? 0;
    },
    enabled: !!session && open && !!userProfile?.org_id,
    staleTime: 5_000
  });
    ...
    onOpenChange(v);
      ...
      <AlertDialogContent>
          ...
          <AlertDialogDescription asChild>
            <div className="space-y-4">
              {isSettling ? <div className="space-y-3 py-2">
                  ...
                  {!balanceLoading && !hasEnough && <div className="flex items-start gap-2 p-3 rounded-md bg-destructive/10 text-destructive text-sm">
        ...
        <AlertDialogFooter>
          ...
          <AlertDialogAction onClick={onConfirm} disabled={isSettling || balanceLoading || !hasEnough}>
            {isSettling ? <>
