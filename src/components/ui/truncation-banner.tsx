...
export function TruncationBanner({
  ...
  const isTruncated = resolvedTotal != null && resolvedLimit != null && resolvedTotal > resolvedLimit || resolvedTotal == null && resolvedLimit != null && rowCount >= resolvedLimit;
  if (!isTruncated) return null;
  const displayTotal = resolvedTotal != null ? resolvedTotal.toLocaleString() : "more";
  return <div className="flex items-center gap-2 rounded-md border border-warning/30 bg-warning/5 px-3 py-2 text-sm text-warning-foreground">
      <AlertTriangle className="h-4 w-4 flex-shrink-0 text-warning" />
      <span>
        Showing {rowCount.toLocaleString()} of {displayTotal} records.
        {resolvedTotal != null && resolvedLimit != null && <> Use filters or pagination to see all results.</>}
        {resolvedTotal == null && <> Results may be incomplete: refine your query to see all data.</>}
      </span>
    </div>;
}