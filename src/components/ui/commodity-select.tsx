    ...
    <div ref={containerRef} className={cn("relative", className)}>
      <div className="relative">
        <input
          ...
          autoComplete="off"
          className={cn(
            "w-full pr-8 transition-all duration-200",
            isLanding
              ? [
                  "h-10 px-3 text-[13px] font-mono rounded-lg",
        ...
        <ChevronsUpDown
          className={cn(
            ...
            isLanding ? "text-[var(--lt-text-dim)]" : "text-muted-foreground"
          )}
        />
      </div>
      ...
      {open && !disabled && (
        <div
          ref={listRef}
          className={cn(
            "absolute z-50 mt-1 w-full max-h-64 overflow-y-auto rounded-lg border shadow-lg",
            isLanding
              ? "bg-[#111827] border-[var(--lt-border)]"
        ...
        >
          {flatResults.length === 0 ? (
            ...
            >
              <Package className="h-3.5 w-3.5 inline mr-1.5 opacity-60" />
              No match found — your text will be used as-is
            </div>
          ) : (
            Array.from(grouped.entries()).map(([category, items]) => (
              <div key={category}>
                ...
                <div
                  className={cn(
                    "px-3 py-1.5 text-[10px] font-mono uppercase tracking-wider font-semibold sticky top-0",
                    isLanding
                      ? "text-[var(--lt-text-dim)] bg-[#0D1220] border-b border-[var(--lt-border)]"
                      : "text-muted-foreground bg-muted/50 border-b"
                  )}
                >
                  {category}
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        ...
                        isLanding
                          ? [
                              isHighlighted ? "bg-[var(--lt-panel)]" : "",
                              isSelected ? "text-[var(--lt-emerald)]" : "text-[var(--lt-text)]",
                            ].join(" ")
                          : [
