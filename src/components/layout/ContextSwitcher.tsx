...
export function ContextSwitcher({ tone = "light", className = "" }: ContextSwitcherProps) {
  const { isPlatformAdmin, isOrgAdmin, roles } = useAuth();
  ...
  const active =
    available.find((w) => w.matches.some((m) => pathname === m || pathname.startsWith(m + "/"))) ??
    available[0];
  ...
  const triggerClasses = isDark
    ? "w-full flex items-center gap-2.5 px-3 py-2 rounded-sm border border-slate-800 bg-slate-950 hover:bg-slate-900 transition-colors text-left"
    : "w-full flex items-center gap-2.5 px-3 py-2 rounded-md border border-slate-200 bg-white hover:bg-slate-50 transition-colors text-left";
  const labelClasses = isDark
    ? "font-mono text-[11px] uppercase tracking-[0.2em] text-slate-500"
    : "font-mono text-[10px] uppercase tracking-[0.25em] text-slate-400";
  const valueClasses = isDark
    ? "text-[13px] text-slate-100 font-medium tracking-tight"
    : "text-[13px] text-slate-900 font-medium tracking-tight";
  const chevronClasses = isDark ? "h-3.5 w-3.5 text-slate-500" : "h-3.5 w-3.5 text-slate-400";
  const iconWrapClasses = isDark
    ? "h-7 w-7 rounded-sm bg-slate-900 border border-slate-800 flex items-center justify-center"
    : "h-7 w-7 rounded-md bg-slate-50 border border-slate-200 flex items-center justify-center";
    ...
    <div className={className}>
      <DropdownMenu>
        ...
        <DropdownMenuContent
          align="start"
          sideOffset={6}
          className="w-[260px] p-1.5 bg-white border border-slate-200 shadow-lg"
        >
          ...
          {available.map((w) => {
            ...
            return (
              <DropdownMenuItem
                ...
                className={[
                  "flex items-start gap-2.5 px-2 py-2 rounded-sm cursor-pointer",
