...
function GettingStartedEmpty({
  ...
  return <div className="space-y-8">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        {steps.map(step => <button key={step.number} onClick={step.action} className="relative p-5 rounded-lg border border-border bg-background hover:border-primary/40 hover:bg-accent/30 transition-colors text-left group">
...
export function ConsoleOverview() {
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("profiles").select("org_id").eq("id", session!.user.id).single();
  ...
  const {
    ...
    queryFn: async () => {
      const {
        data,
        error
      } = await supabase.from("token_balances").select("balance, minimum_required").eq("org_id", userProfile!.org_id).single();
      if (error) throw error;
      return data;
    },
  ...
  const {
    ...
    queryFn: async () => {
      ...
      const [apiKeys, logs24h, logs7d, matches, lastLog] = await Promise.all([supabase.from("api_keys").select("id", {
        count: "exact",
        head: true
      }).eq("status", "active"), supabase.from("api_request_logs").select("id", {
        count: "exact",
        head: true
      }).gte("created_at", last24h), supabase.from("api_request_logs").select("id", {
        count: "exact",
        head: true
      }).gte("created_at", last7d), supabase.from("matches").select("id", {
        count: "exact",
        head: true
      }).eq("status", "confirmed"), supabase.from("api_request_logs").select("created_at").order("created_at", {
        ascending: false
      }).limit(1).maybeSingle()]);
  ...
  const statCards = [{
    ...
    label: "Calls (24h)",
    value: stats?.callsLast24h ?? 0,
    icon: Activity
  }, {
    label: "Calls (7d)",
    value: stats?.callsLast7d ?? 0,
    icon: BarChart3
  }, {
    label: "Trade Requests",
    value: stats?.confirmedIntents ?? 0,
    icon: FileText
  }];
  const hasZeroActivity = !isLoading && stats && stats.activeApiKeys === 0 && stats.callsLast24h === 0 && stats.callsLast7d === 0 && stats.confirmedIntents === 0 && !stats.lastActivity;
  return <div className="space-y-6">
      ...
      {isError && <div className="p-6 border border-destructive/30 rounded-lg bg-destructive/5 text-center">
      ...
      if (!hasKeys) {
        return <div className="p-4 border border-primary/30 rounded-lg bg-primary/5 flex items-center justify-between gap-4">
              <div>
      ...
      {!isError && tokenBalance && <Link to="/billing" className={cn("flex items-center justify-between p-4 rounded-lg border transition-colors hover:bg-accent/30", isBalanceCritical ? "border-destructive/50 bg-destructive/5" : isBalanceLow ? "border-amber-500/50 bg-amber-500/5" : "border-primary/30 bg-primary/5")}>
          <div className="flex items-center gap-3">
            <div className={cn("flex h-10 w-10 items-center justify-center rounded-full", isBalanceCritical ? "bg-destructive/10" : isBalanceLow ? "bg-amber-500/10" : "bg-primary/10")}>
              <Coins className={cn("h-5 w-5", isBalanceCritical ? "text-destructive" : isBalanceLow ? "text-amber-600" : "text-primary")} />
            </div>
            <div>
              <p className="text-sm text-muted-foreground">Credit Balance</p>
              <p className={cn("text-2xl font-bold", isBalanceCritical ? "text-destructive" : isBalanceLow ? "text-amber-600" : "text-foreground")}>
          ...
          <div className="text-right">
            ...
            {isBalanceCritical && <p className="text-xs font-medium text-destructive mt-1">Top up now →</p>}
            {isBalanceLow && !isBalanceCritical && <p className="text-xs font-medium text-amber-600 mt-1">Running low →</p>}
            {!isBalanceLow && <p className="text-xs text-muted-foreground mt-1">Purchase credits →</p>}
          </div>
        </Link>}

      {/* Stats Grid - only render when we have data or are loading (not on error) */}
      {!isError && <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {statCards.map(stat => <div key={stat.label} className="p-4 border border-border rounded-lg bg-background">
              <div className="flex items-center gap-2 mb-2">
                <stat.icon className="h-4 w-4 text-muted-foreground" />
                <span className="text-xs text-muted-foreground">{stat.label}</span>
              </div>
              {isLoading ? <Skeleton className="h-8 w-16" /> : <p className="text-2xl font-semibold text-foreground">
      ...
      {!isError && !isLoading && <KYCStatusCard />}

      {/* Last Activity */}
      {!isError && !hasZeroActivity && <div className="flex items-center gap-2 text-sm text-muted-foreground">
      ...
      {!isError && hasZeroActivity ? <GettingStartedEmpty onStartWizard={() => setWizardOpen(true)} /> : !isError && !hasZeroActivity && !isLoading ? <div className="space-y-4">
          <div className="p-5 border border-border rounded-lg bg-muted/30">
            ...
            desc: "Seal evidence"
          }].map(s => <div key={s.step} className="text-center p-3 rounded-md bg-background border border-border">
