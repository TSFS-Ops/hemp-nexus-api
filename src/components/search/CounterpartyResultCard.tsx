...
import { CheckCircle, ExternalLink, Sparkles, Users, Lightbulb, Globe, Shield, Mail } from "lucide-react";
...
function getSourceLabel(source: string): {
  ...
  switch (source) {
    ...
    case "order_book":
      return {
        label: "Order Book",
        icon: <Sparkles className="h-2.5 w-2.5 sm:h-3 sm:w-3" />,
        color: "bg-purple-100 text-purple-700 dark:bg-purple-900/50 dark:text-purple-300"
      };
    case "web_discovery":
      return {
        label: "Web Discovery",
        icon: <Globe className="h-2.5 w-2.5 sm:h-3 sm:w-3" />,
...
export function CounterpartyResultCard({
  ...
  return <Card className={`transition-all cursor-pointer hover:border-primary/50 ${isSelected ? "border-primary bg-primary/5 ring-1 ring-primary/20" : ""}`} onClick={() => onToggleSelect(result.id)}>
      <CardContent className="p-3 sm:p-4">
        <div className="flex gap-3">
          {/* Rank/Selection indicator */}
          <div className="flex-shrink-0 flex flex-col items-center gap-0.5">
            <div className={`w-8 h-8 sm:w-9 sm:h-9 rounded-full flex items-center justify-center text-sm font-semibold transition-colors ${isSelected ? "bg-primary text-primary-foreground" : "bg-muted text-muted-foreground"}`}>
              {isSelected ? <CheckCircle className="h-4 w-4" /> : rank}
            </div>
          ...
          <div className="flex-1 min-w-0 space-y-1.5">
            ...
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="flex items-center gap-1 flex-shrink-0">
                ...
                <Badge variant="secondary" className={`${sourceInfo.color} text-[10px] sm:text-xs px-1.5 py-0 h-5 gap-0.5`}>
                  {sourceInfo.icon}
            ...
            {isWebDiscovered && hasContact && <div className="flex items-center gap-1.5 text-xs text-muted-foreground">
            ...
            <div className="flex items-center gap-2 flex-wrap pt-0.5">
              <div className="flex items-center gap-1">
                <div className={`w-1.5 h-1.5 sm:w-2 sm:h-2 rounded-full ${result.coherence.passed ? "bg-green-500" : "bg-yellow-500"}`} />
              ...
              {result.coherence.factors.length > 0 && <>
                  <Badge variant="outline" className="text-[10px] sm:text-xs py-0 h-4 sm:h-5 px-1.5">
                    {result.coherence.factors[0]}
                  </Badge>
                  {result.coherence.factors.length > 1 && <Badge variant="outline" className="text-[10px] sm:text-xs py-0 h-4 sm:h-5 px-1.5 hidden sm:inline-flex">
                      {result.coherence.factors[1]}
                    </Badge>}
                  {result.coherence.factors.length > 2 && <span className="text-[10px] text-muted-foreground hidden sm:inline">
                      +{result.coherence.factors.length - 2}
                    </span>}
                  {result.coherence.factors.length > 1 && <span className="text-[10px] text-muted-foreground sm:hidden">
            ...
            <div className="flex items-center gap-1.5 sm:gap-2 pt-1.5 sm:pt-2">
              {result.url !== "#" && <Button variant="ghost" size="sm" className="h-8 sm:h-7 text-xs px-2 sm:px-3 touch-target" asChild>
              ...
              <Tooltip>
                <TooltipTrigger asChild>
                  <Button variant="ghost" size="sm" className="h-8 sm:h-7 text-xs px-2 sm:px-3 touch-target" onClick={e => {
                  e.stopPropagation();
