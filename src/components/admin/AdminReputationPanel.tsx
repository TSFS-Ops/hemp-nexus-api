...
type ReputationScore = Tables<"reputation_scores">;
...
export function AdminReputationPanel() {
  ...
  const fetchScores = async () => {
    ...
    setError(null);
    try {
      const { count } = await supabase.from("reputation_scores").select("id", { count: "exact", head: true });
      setTotal(count);

      let query = supabase.from("reputation_scores").select("*").order("overall_score", { ascending: false }).limit(QUERY_LIMIT_ADMIN);
      if (search.trim()) query = query.eq("org_id", search.trim());
  ...
  if (error && scores.length === 0) return <ErrorState title="Failed to load reputation scores" message={error} onRetry={fetchScores} />;
      ...
      <CardContent className="space-y-4">
        <div className="flex items-center gap-3">
          <div className="relative flex-1">
            ...
            <Input placeholder="Filter by org ID…" value={search} onChange={(e) => setSearch(e.target.value)} onKeyDown={(e) => e.key === "Enter" && fetchScores()} className="pl-9" />
          </div>
        ...
        {total !== null && scores.length >= QUERY_LIMIT_ADMIN && (
          <Alert><AlertTriangle className="h-4 w-4" /><AlertDescription>Showing {scores.length} of {total} scores.</AlertDescription></Alert>
        )}

        {loading && scores.length === 0 ? <InlineLoader message="Loading reputation scores…" /> : scores.length === 0 ? (
          <div className="text-center py-8 text-muted-foreground"><TrendingUp className="h-8 w-8 mx-auto mb-2 opacity-50" /><p className="text-sm">No reputation scores computed yet.</p></div>
        ) : (
          <div className="overflow-x-auto">
            <Table>
              <TableHeader><TableRow>
                <TableHead>Org ID</TableHead><TableHead>Overall</TableHead><TableHead>Level</TableHead><TableHead>Completed</TableHead><TableHead>Failed</TableHead><TableHead>Reliability</TableHead><TableHead>Computed</TableHead>
