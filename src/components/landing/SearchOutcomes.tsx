...
export function SearchOutcomes({
  isSearching, hasSearched, liquidityData, onSignIn,
}: SearchOutcomesProps) {
      ...
      <div className="animate-fade-up" style={{ borderTop: '1px solid var(--lt-border)' }}>
        <div className="p-4 sm:p-5">
          <div className="flex items-center gap-2 mb-2">
            <SearchX className="h-4 w-4 flex-shrink-0" style={{ color: 'var(--lt-text-muted)' }} />
            <h3 className="text-[15px] font-semibold tracking-tight leading-tight" style={{ color: 'var(--lt-text)' }}>
              No active partners yet
            </h3>
          </div>
          <p className="text-[12px] font-medium leading-relaxed mb-4 max-w-md" style={{ color: 'var(--lt-text-muted)' }}>
            We don't have verified partners for this product right now. Create a free account to register your trade interest — you'll be notified when a match becomes available.
          </p>
          <button
            onClick={onSignIn}
            ...
            style={{ backgroundColor: 'var(--lt-panel)', color: 'var(--lt-text)', border: '1px solid var(--lt-border)' }}
          >
