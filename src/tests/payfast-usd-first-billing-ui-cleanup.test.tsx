 ...
 * 2. Paystack rows are unaffected (for the admin viewer, which is the
 *    only viewer that still sees the "via Paystack" wording -- see
 *    payfast-frontend-trust-cleanup.test.tsx for the non-admin gate).
...
// PurchasesList now gates legacy Paystack wording behind an admin role
// check. This suite is about USD-first wording, not admin gating, so we
// render as an admin to preserve the original "via Paystack" assertion.
