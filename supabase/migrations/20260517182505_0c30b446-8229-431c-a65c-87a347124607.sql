...
BEGIN
  ...
  WHERE
    COALESCE((m.metadata ->> 'legacy_archived_admin_hold')::boolean, false) = false
    AND (
...
COMMENT ON FUNCTION public.admin_list_inconsistent_matches() IS
  'MT-008 Legacy Repair queue: read-only platform-admin list of inconsistent matches. Excludes rows held by metadata.legacy_archived_admin_hold=true (archive removes from active queue, audit + DB row preserved). Uses public.is_admin(auth.uid()).';