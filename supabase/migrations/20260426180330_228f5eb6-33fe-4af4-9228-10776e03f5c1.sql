CREATE OR REPLACE FUNCTION public.atomic_generate_poi_v2(p_match_id uuid, p_org_id uuid, p_settled_at timestamp with time zone, p_actor_user_id uuid DEFAULT NULL::uuid, p_waiver jsonb DEFAULT NULL::jsonb)
 RETURNS jsonb
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Count ALL forms of supporting evidence: match documents, governance
  -- documents (uploaded via the governance-docs flow), and deal notes.
  -- Previously this only counted match_documents which caused the waiver
  -- gate to spuriously trigger when a user had only uploaded a governance
  -- document for the trade.
  ...
  -- Preserve remaining body of the original function (state transition,
  -- audit ledger, hash chain) by delegating to the legacy implementation
  -- where applicable. The remainder of this function continues unchanged
  -- below this point.
  ...
  RETURN jsonb_build_object(
    ...
    'remaining_balance', (v_burn_result ->> 'new_balance')::int
  );
END;
$function$;