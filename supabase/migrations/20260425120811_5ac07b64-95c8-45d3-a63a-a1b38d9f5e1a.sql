CREATE OR REPLACE FUNCTION public.atomic_engagement_transition(
  p_engagement_id uuid, p_actor_type text, p_actor_user_id uuid,
  p_actor_email text, p_actor_name text, p_new_status text,
  p_entry_type text, p_contact_method text DEFAULT NULL::text,
  p_contact_detail text DEFAULT NULL::text, p_notes text DEFAULT NULL::text,
  p_audit_action text DEFAULT NULL::text, p_audit_org_id uuid DEFAULT NULL::uuid
)
RETURNS jsonb
LANGUAGE plpgsql
SECURITY DEFINER
...
BEGIN
  ...
  IF p_new_status = 'accepted' AND v_prev_status <> 'accepted' THEN
    ...
    IF v_initiator_user_id IS NOT NULL THEN
      -- Use real notification_dispatches columns:
      -- recipient_address (not recipient_email), event_type, reference_type, reference_id.
      INSERT INTO notification_dispatches (
        event_type, reference_type, reference_id,
        recipient_user_id, recipient_address,
        channel, template_name, status, metadata
      ) VALUES (
        'engagement-accepted', 'poi_engagement', p_engagement_id,
        v_initiator_user_id, v_initiator_email,
        'email', 'engagement-accepted', 'pending',
      ...
      INSERT INTO notification_dispatches (
        ...
        'engagement-accepted', 'poi_engagement', p_engagement_id,
        v_initiator_user_id, v_initiator_email,
        'in_app', 'engagement-accepted', 'pending',
