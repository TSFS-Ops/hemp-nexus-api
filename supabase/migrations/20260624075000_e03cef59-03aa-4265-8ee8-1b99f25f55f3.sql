...
DECLARE
  ...
  -- Inclusive upper bound used by B3 to cover backfill-stamped rows.
...
BEGIN
  ...
  -- B3 cutoff-inclusive residual auto-resolve pass.
  -- Closes only historical/cutoff-boundary "not notified" alarms via one of
  -- three evidence branches. Receipts strictly after the inclusive cutoff
  -- are never touched here.
  --
  -- Branch 1: in-app delivered/opened + terminal email send_unverifiable.
  -- Branch 2: matching email_send_log evidence (any recipient address
  --           present on receipt or its email dispatch).
  -- Branch 3: cutoff-boundary artefact with no recipient on receipt, no
  --           notification_dispatches row at all, and no email_send_log
  --           evidence in window.
  PERFORM set_config('app.allow_risk_item_update', 'on', true);
  ...
  WITH to_resolve_b3b AS (
    ...
    WHERE ari.status = 'open'
      ...
      AND ar.created_at <= v_inclusive_backfill_cutoff
      -- Exclude rows already covered by Branch 1 this run.
