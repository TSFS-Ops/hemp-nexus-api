-- Guard: this is a data-004-batch13 cold-storage-positive-live fixture proof
-- that depends on an out-of-band organization existing. Skip safely on clean
-- disposable-DB replay when that organization is not present.
DO $guard$
BEGIN
IF NOT EXISTS (SELECT 1 FROM public.organizations WHERE id = '8fc9ee52-ce88-456f-8ef9-c6984fc6fae1'::uuid) THEN
RAISE NOTICE 'Proof prerequisite organization not present; cold-storage retention fixture skipped during clean replay.';
RETURN;
END IF;
