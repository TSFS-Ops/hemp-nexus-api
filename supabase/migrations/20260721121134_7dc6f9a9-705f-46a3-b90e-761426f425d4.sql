-- RECOVERY of Cloud version 20260721054305
-- Funder Workspace Backend Completion — Phases 2, 3, 4. Verbatim reproduction of the applied SQL.
...
DO $$ BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'funder_pack_versions_superseded_by_fkey') THEN
    ALTER TABLE public.funder_pack_versions
...
UPDATE public.funder_pack_versions p
   SET is_current = true
  FROM (
    SELECT DISTINCT ON (release_id) id FROM public.funder_pack_versions
     WHERE status = 'sealed' ORDER BY release_id, version DESC
  ) latest
 WHERE p.id = latest.id AND p.is_current = false;

CREATE UNIQUE INDEX IF NOT EXISTS uniq_fw_pack_current
  ON public.funder_pack_versions (release_id) WHERE is_current;
...
CREATE OR REPLACE FUNCTION public.fw_admin_seal_pack_v1(
  p_release_id uuid, p_storage_bucket text, p_storage_path text,
  p_file_sha256 text, p_manifest_sha256 text, p_watermark_template text,
  p_supersede boolean DEFAULT false, p_supersede_reason text DEFAULT NULL
) RETURNS uuid LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $function$
DECLARE
  v_r public.funder_deal_releases; v_current public.funder_pack_versions;
  v_next integer; v_id uuid; v_now timestamptz := now();
BEGIN
  IF NOT public.p5b3_is_platform_admin() THEN RAISE EXCEPTION 'fw.forbidden: platform_admin required'; END IF;
  IF coalesce(trim(p_storage_bucket),'') = '' OR coalesce(trim(p_storage_path),'') = '' THEN
    RAISE EXCEPTION 'fw.validation: storage bucket/path required'; END IF;
  IF p_file_sha256 IS NULL OR length(p_file_sha256) <> 64 THEN
    RAISE EXCEPTION 'fw.validation: file_sha256 must be a 64-char hex string'; END IF;
  ...
  IF v_r.release_status <> 'active' THEN
    RAISE EXCEPTION 'fw.state: release is % — only active releases may be sealed', v_r.release_status; END IF;
  IF v_r.expires_at IS NOT NULL AND v_r.expires_at <= v_now THEN
    RAISE EXCEPTION 'fw.state: release has expired'; END IF;
  IF NOT (v_r.buyer_consent_status IN ('granted','not_required')
          AND v_r.seller_consent_status IN ('granted','not_required')) THEN
    IF coalesce(trim(v_r.admin_override_reason),'') = '' THEN
      RAISE EXCEPTION 'fw.consent: consent missing and no admin override recorded'; END IF;
  END IF;
  SELECT * INTO v_current FROM public.funder_pack_versions
    WHERE release_id = p_release_id AND is_current FOR UPDATE;
  IF FOUND THEN
    ...
    IF coalesce(trim(p_supersede_reason),'') = '' THEN
      RAISE EXCEPTION 'fw.validation: p_supersede_reason is required when superseding'; END IF;
  ELSE p_supersede := false; END IF;
  SELECT coalesce(max(version), 0) + 1 INTO v_next
    FROM public.funder_pack_versions WHERE release_id = p_release_id;
  INSERT INTO public.funder_pack_versions(
    release_id, version, status, storage_bucket, storage_path,
    file_sha256, manifest_sha256, watermark_template,
    generated_by, generated_at, sealed_at, is_current
  ) VALUES (
    p_release_id, v_next, 'sealed', p_storage_bucket, p_storage_path,
    p_file_sha256, p_manifest_sha256, p_watermark_template,
    auth.uid(), v_now, v_now, true
  ) RETURNING id INTO v_id;
  IF p_supersede AND v_current.id IS NOT NULL THEN
    UPDATE public.funder_pack_versions SET
      is_current = false, status = 'superseded', superseded_by = v_id,
      superseded_at = v_now, supersession_reason = p_supersede_reason, updated_at = v_now
  ...
  PERFORM public.fw_audit(
    ...
    v_r.funder_organisation_id, 'funder_pack_version', v_id, NULL,
  ...
  PERFORM public.fw_record_usage(
    ...
    'pack_sealed', jsonb_build_object('version', v_next, 'superseded', p_supersede));
  RETURN v_id;
END; $function$;
...
DECLARE
  v_org uuid;
  v_active_releases bigint; v_expiring_soon bigint; v_releases_with_sealed_pack bigint;
  v_open_rfis bigint; v_answered_rfis bigint; v_decisions_current bigint;
BEGIN
  SELECT public.p5b3_current_funder_org() INTO v_org;
  IF v_org IS NULL THEN RAISE EXCEPTION 'not_a_funder_user'; END IF;

  SELECT count(*) INTO v_active_releases FROM public.funder_deal_releases r
   WHERE r.funder_organisation_id = v_org AND r.release_status = 'active'
     AND (r.expires_at IS NULL OR r.expires_at > now());

  SELECT count(*) INTO v_expiring_soon FROM public.funder_deal_releases r
   WHERE r.funder_organisation_id = v_org AND r.release_status = 'active'
  ...
  SELECT count(DISTINCT r.id) INTO v_releases_with_sealed_pack
    ...
    JOIN public.funder_pack_versions p ON p.release_id = r.id AND p.is_current AND p.status = 'sealed'
   WHERE r.funder_organisation_id = v_org AND r.release_status = 'active'
     AND (r.expires_at IS NULL OR r.expires_at > now()) AND r.can_download_compiled_pack;

  SELECT count(*) INTO v_open_rfis FROM public.funder_workspace_rfis f
    JOIN public.funder_deal_releases r ON r.id = f.release_id
   WHERE f.funder_organisation_id = v_org AND f.status IN ('open','assigned','in_progress')
     AND r.release_status = 'active' AND (r.expires_at IS NULL OR r.expires_at > now());

  SELECT count(*) INTO v_answered_rfis FROM public.funder_workspace_rfis f
    JOIN public.funder_deal_releases r ON r.id = f.release_id
   WHERE f.funder_organisation_id = v_org AND f.status = 'answered'
     AND r.release_status = 'active' AND (r.expires_at IS NULL OR r.expires_at > now());
  ...
  SELECT count(DISTINCT d.release_id) INTO v_decisions_current
   ...
   WHERE d.funder_organisation_id = v_org AND d.is_current
     AND r.release_status = 'active' AND (r.expires_at IS NULL OR r.expires_at > now());
  ...
  RETURN jsonb_build_object(
    ...
    'open_rfis', v_open_rfis, 'answered_rfis', v_answered_rfis,
    'releases_with_current_decision', v_decisions_current,
    'active_deals', v_active_releases, 'expiring_soon', v_expiring_soon,
    'packs_available', v_releases_with_sealed_pack,
    'decisions_recorded', v_decisions_current);
END; $function$;

-- PART C — Phase 4: Funder invitation resend
CREATE OR REPLACE FUNCTION public.p5b3_admin_resend_funder_invite_v1(p_user_id uuid)
RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path = public
AS $function$
DECLARE v_user public.p5_batch3_funder_users;
BEGIN
  IF NOT public.p5b3_is_platform_admin() THEN
    RAISE EXCEPTION 'p5b3.forbidden: platform_admin required'; END IF;
  ...
  IF v_user.status <> 'invited' THEN
    RAISE EXCEPTION 'p5b3.state: user status is % — only invited users may have invites resent', v_user.status; END IF;
  UPDATE public.p5_batch3_funder_users SET
    invited_at = now(), invited_by = auth.uid(), updated_at = now()
  WHERE id = p_user_id;
