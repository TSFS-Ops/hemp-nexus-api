...
DECLARE
  ...
  v_result jsonb;
BEGIN
  -- 1. Lock the match row and validate state
  SELECT * INTO v_match
  FROM matches
  WHERE id = p_match_id
  FOR UPDATE;
  ...
  -- 2. Transition the match to completed
  UPDATE matches
  SET state = 'completed',
      status = 'completed',
      settled_at = now()
  ...
  -- 3. Build and insert the hash-chained event
  ...
  -- Update the match chain hash
  UPDATE matches SET event_chain_hash = v_payload_hash WHERE id = p_match_id;

  -- 4. Write collapse ledger entry (if payload provided)
  ...
  -- 5. Audit log (within the same transaction)
  ...
  -- 6. Return success with all references
  ...
  -- If ANY of the above fails, PostgreSQL automatically rolls back the entire transaction
END;
$$;
