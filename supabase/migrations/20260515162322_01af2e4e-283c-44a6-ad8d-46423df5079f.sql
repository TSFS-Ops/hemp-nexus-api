-- Batch O Phase 2 — read-only admin RPC for MT-008 Legacy Repair queue.
-- Returns inconsistent match rows across all orgs. Admin-only.
-- No mutations, no audit writes (deferred to Phase 2b for idempotency safety).
...
AS $$
BEGIN
  -- Admin-only. Mirrors all other admin-cross-org reads.
  ...
  SELECT
    ...
    ARRAY_REMOVE(ARRAY[
      CASE WHEN m.status = 'settled' AND m.poi_state = 'DRAFT'
        THEN 'settled_status_with_draft_poi' END,
  ...
  WHERE
    ...
    OR (m.buyer_committed_at IS NOT NULL
        AND m.seller_committed_at IS NOT NULL
        AND m.state = 'discovery')
    OR (m.metadata ->> 'legacy_repair_required') = 'true'
...
COMMENT ON FUNCTION public.admin_list_inconsistent_matches() IS
  'Batch O Phase 2 (MT-008): admin-only read-only list of matches whose lifecycle fields are mutually inconsistent. Mirrors src/lib/match-lifecycle.ts isInconsistentMatch.';