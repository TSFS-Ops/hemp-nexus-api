-- RECOVERED from Cloud version 20260721005020 (drift-audit reconciliation).
...
BEGIN
  ...
  IF v_changed THEN
    ...
    PERFORM public.fw_audit(
      'funder_deal.permissions_updated', v_r.funder_organisation_id,
      'funder_deal_release', p_release_id, v_old, v_new, v_reason
    );
  END IF;
  RETURN jsonb_build_object(
    'release_id', p_release_id, 'changed', v_changed, 'old', v_old, 'new', v_new
  );
END;
$$;
