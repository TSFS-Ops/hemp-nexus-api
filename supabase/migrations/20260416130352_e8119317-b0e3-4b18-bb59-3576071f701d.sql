CREATE OR REPLACE FUNCTION public.atomic_validate_governance_doc(
  p_governance_doc_id uuid,
  p_org_id uuid,
  p_burn_amount integer,
...
DECLARE
  ...
  v_burn_result jsonb;
BEGIN
  -- Lock the governance document row
  ...
  -- Token burn if required (inside same transaction)
  IF p_burn_amount > 0 AND NOT v_doc.token_burned THEN
    ...
    -- Audit log for burn (same transaction)
  ...
  -- Update status (same transaction — if this fails, burn is rolled back)
