-- D-02 follow-up: make p_terms_hash mandatory on POI mint.
-- Rejects NULL, blank, or non-hex values with stable TERMS_HASH_REQUIRED.
-- TERMS_DRIFT remains for well-formed hashes that don't match the server.
...
BEGIN
  ...
  -- ── D-02 FOLLOW-UP: TERMS HASH IS NOW MANDATORY ──
  -- Previously p_terms_hash was optional for backwards-compat. That left a
  -- silent bypass: any caller (legacy script, broken edge path, missed
  -- frontend wire) could mint without a hash and the server would accept
  -- it with terms_hash_supplied=false in the audit. Closed here:
  --   • NULL / blank / wrong-shape  → TERMS_HASH_REQUIRED (no burn)
  --   • well-formed but mismatched  → TERMS_DRIFT (no burn)
  --   • well-formed and matches     → continue to mint
  ...
  -- Recompute the canonical fingerprint from the live row under the row
  -- lock taken above (FOR UPDATE).
...
-- Re-apply Stage D1 lockdown grants on the new function body.
