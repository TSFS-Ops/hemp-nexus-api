...
BEGIN
  -- Lock the match row for the duration of the transaction
  ...
  -- ── EVIDENCE WAIVER GATE (atomic, server-authoritative) ──
  -- Re-count docs/notes UNDER LOCK so the waiver decision matches reality at
  -- mint time. This closes the client-stale-counts race.
  ...
  IF v_waiver_required THEN
    -- Look for an existing acknowledged waiver in audit_logs
    ...
    IF v_existing_waiver IS NULL AND NOT v_waiver_supplied THEN
      -- No prior waiver and none supplied this call → block
    ...
    IF v_waiver_supplied THEN
      -- Validate the supplied waiver payload
      ...
      END IF;
      IF length(coalesce(p_waiver->>'reason', '')) < 10 THEN
        RETURN jsonb_build_object('success', false, 'error', 'WAIVER_INVALID', 'message', 'Waiver reason must be at least 10 characters.');
      END IF;
    ...
    END IF;
  ELSE
    -- Evidence exists → reject any waiver payload outright (false waiver)
    IF v_waiver_supplied THEN
      RETURN jsonb_build_object(
        'success', false,
  ...
  -- Burn token (rolls back if anything below fails)
  ...
  -- Write the waiver audit row INSIDE the same transaction as the burn + state change
