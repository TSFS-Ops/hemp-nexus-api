...
DROP INDEX IF EXISTS public.idx_idempotency_keys_lookup;
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_matches_org_hash ON public.matches(org_id, hash);
...
BEGIN
  IF NOT EXISTS (
    ...
    AND policyname = 'Service role can manage idempotency keys'
  ) THEN
