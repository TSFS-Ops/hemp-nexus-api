-- RECOVERED from Cloud version 20260718234531 (drift-audit reconciliation).
-- Original migration was applied to Cloud but the local SQL file was missing.
-- This is a byte-faithful, idempotent replay: fixtures use ON CONFLICT DO NOTHING
-- and the RPC uses CREATE OR REPLACE, so re-application is a no-op on Cloud.
