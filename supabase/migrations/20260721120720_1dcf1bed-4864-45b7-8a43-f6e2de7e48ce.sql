-- RECOVERED from Cloud version 20260721054305 (drift-audit reconciliation).
-- Fully idempotent: ADD COLUMN IF NOT EXISTS, guarded FK creation,
-- CREATE OR REPLACE FUNCTION, DROP TRIGGER IF EXISTS + CREATE TRIGGER,
-- and a backfill UPDATE whose target rows are already flagged is_current=true.
  ...
  IF p_supersede AND v_current.id IS NOT NULL THEN
    UPDATE public.funder_pack_versions
       SET is_current = false, status = 'superseded',
           superseded_by = v_id, superseded_at = v_now,
           supersession_reason = p_supersede_reason, updated_at = v_now
...
BEGIN
  ...
  UPDATE public.p5_batch3_funder_users
     SET invited_at = now(), invited_by = auth.uid(), updated_at = now()
   WHERE id = p_user_id;
