-- Fix atomic_engagement_transition: it inserts notification_dispatches rows
-- with status='queued', which violates notification_dispatches_status_check
-- (allowed: pending, dispatched, delivered, opened, failed, suppressed).
-- The dispatcher polls status='pending', so 'pending' is the correct value.
-- Symptom before fix: counterparty Accept Trade returns 500
-- "An internal error occurred"; no acceptance receipt or attestation is
-- written. Engagement is stuck at 'contacted'.
...
BEGIN
  ...
  IF p_new_status = 'accepted' AND v_prev_status <> 'accepted' THEN
    ...
    IF v_initiator_user_id IS NOT NULL THEN
      -- FIX: was 'queued' which violated notification_dispatches_status_check.
      -- The dispatcher (dispatch-acceptance-receipts) polls status='pending'.
      ...
      (v_initiator_user_id, v_initiator_email, 'email', 'engagement-accepted',
       'poi_engagement', p_engagement_id, 'pending',
