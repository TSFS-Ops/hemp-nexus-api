...
// Batch F4: business mutation (compliance_holds release + linked OVR closure
// + canonical legacy audit row) + Governance Record event run atomically
// via admin_compliance_hold_release_with_governance.
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, { adminClient: admin, callerUserId: admin_user.id, action: "admin-compliance-hold-release" });
  } catch (e) {
  ...
  const { data, error } = await admin.rpc("admin_compliance_hold_release_with_governance", {
    p_hold_id: hold_id,
  ...
  if (error) {
    console.error("[admin-compliance-hold-release] atomic rpc failed:", error);
    return json({ error: "rpc_failed", code: "ATOMIC_COMPLIANCE_HOLD_RELEASE_FAILED", message: error.message }, 500);
  }
  ...
  if (!r?.success) {
    ...
    const status = code === "NOT_FOUND" ? 404 : code === "HOLD_NOT_ACTIVE" ? 409 : 400;
    return json({ error: code.toLowerCase(), code }, status);
  }
  return json({ ok: true, hold_id: r.hold_id ?? hold_id, status: r.status ?? "released", governance_event_id: r.event_id, deduplicated: !!r.deduplicated }, 200);
});
