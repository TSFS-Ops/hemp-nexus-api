...
// atomically via admin_residency_review_approve_with_governance.
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, { adminClient: admin, callerUserId: u.user.id, action: "data_009.approve_residency_review" });
  } catch (e) {
  ...
  const { data, error } = await admin.rpc("admin_residency_review_approve_with_governance", {
  ...
  if (error) {
    ...
    console.error("[admin-residency-review-approve] atomic rpc failed:", error);
    return json({ error: "rpc_failed", code: "ATOMIC_RESIDENCY_REVIEW_APPROVE_FAILED", message: msg }, 500);
  }
  const r = data as { success?: boolean; deduplicated?: boolean; event_id?: string; review_id?: string; status?: string };
  return json({ ok: true, review_id: r?.review_id ?? parsed.data.review_id, status: r?.status ?? "approved", governance_event_id: r?.event_id, deduplicated: !!r?.deduplicated }, 200);
});
