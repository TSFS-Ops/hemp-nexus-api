...
const Body = z.object({
  ...
  token_purchase_id: z.string().uuid().optional(),
  provider: z.string().min(1).max(64).default("paystack"),
  provider_dispute_reference: z.string().min(1).max(256),
  credits_issued: z.number().int().min(0),
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, { adminClient: admin, callerUserId: u.user.id, action: "admin-payment-dispute-record" });
  } catch (e) {
