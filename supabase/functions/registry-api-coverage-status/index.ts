// Batch 15 — registry-api-coverage-status. Country coverage safe summary.
...
const BodySchema = z.object({
  ...
  scope: z.string().default("registry.coverage.read"),
  ...
  try {
    ...
    if (!key || key.key_hash !== expectedHash) {
      return json(req, 401, buildResponseEnvelope({
        request_id: requestId, client_id: null, mode, scope: body.scope, endpoint: "coverage-status",
        result_state: "api_client_not_allowed", country: body.country,
      }));
    }
    ...
    const decisions = evaluateApiGates({
      ...
      requested_mode: mode, key_type: (key.key_type ?? "sandbox") as never,
      key_status: key.status as never, granted_scopes: granted, requested_scope: body.scope,
      allowed_countries: client!.allowed_countries ?? [], requested_country: body.country,
      allowed_use_cases: client!.allowed_use_cases ?? [], requested_use_case: body.use_case ?? null,
      rate_limited: false,
    });
    const block = gatesToBlockedReason(decisions);
    if (block) {
      await svc.from("registry_api_blocked_events").insert({
        request_id: requestId, client_id: client!.id, key_id: key.id, endpoint: "coverage-status",
        scope: body.scope, mode, country: body.country,
      ...
      return json(req, 403, buildResponseEnvelope({
        request_id: requestId, client_id: client!.id, mode, scope: body.scope, endpoint: "coverage-status",
        result_state: block.result_state, country: body.country,
      }));
    }

    const { data: cov } = await svc.from("registry_country_coverage")
      .select("coverage_state").eq("country_code", body.country).maybeSingle();
    const state: RegistryApiHardenedResultState = !cov ? "not_found"
      : cov.coverage_state === "production_ready" ? "usable"
      : cov.coverage_state === "seed_only" ? "seed_only"
      : cov.coverage_state === "demo_only" ? "demo_only" : "country_not_ready";

    const env = buildResponseEnvelope({
      request_id: requestId, client_id: client!.id, mode, scope: body.scope, endpoint: "coverage-status",
      result_state: state, country: body.country, readiness_summary: cov?.coverage_state ?? null,
    });
    await svc.from("registry_api_usage_events").insert({
      request_id: requestId, client_id: client!.id, key_id: key.id, endpoint: "coverage-status",
      scope: body.scope, mode, country: body.country, result_state: state, usable: env.usable, status_code: 200,
    ...
    console.error("coverage-status error", e);
