...
// Platform admin only. Requires AAL2 + reason >= 20 chars. Hold remains
// active on decline. Does NOT create any technical hosting/storage/
// migration/backup/export/deletion control.
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, {
      ...
      action: "data_009.decline_residency_review",
    });
  } catch (e) {
  ...
  const { data, error } = await admin.rpc("decline_residency_review", {
