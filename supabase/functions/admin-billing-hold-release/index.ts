// PAY-009 — Admin release org billing hold.
// Batch F4: business mutation + canonical admin.hq_decision_recorded event
// run in a single DB transaction via admin_billing_hold_release_with_governance.
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, { adminClient: admin, callerUserId: u.user.id, action: "admin-billing-hold-release" });
  } catch (e) {
  ...
  const { data, error } = await admin.rpc("admin_billing_hold_release_with_governance", {
  ...
  if (error) {
    console.error("[admin-billing-hold-release] atomic rpc failed:", error);
    return json({ error: "rpc_failed", code: "ATOMIC_BILLING_HOLD_RELEASE_FAILED", message: error.message }, 500);
  }
