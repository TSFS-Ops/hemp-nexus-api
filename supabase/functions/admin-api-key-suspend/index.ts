// Public API V1 · Sandprod Batch 3 — Admin-driven API key suspension.
//
// Marks an api_key as status=suspended with reason + actor. The public
// API V1 gateway already rejects suspended keys with the standard
// `suspended_key` error envelope.
  ...
  try {
    ...
    if (env === "production") {
      ...
      await assertAal2(req.headers.get("authorization"), {
        adminClient: supabase, callerUserId: actorUserId, action: "admin.api_production_key_suspend",
      });
    }

    if (existing.status === "suspended") {
      return new Response(JSON.stringify({ ok: true, already: "suspended" }),
        { status: 200, headers: { "Content-Type": "application/json", ...headers } });
    }
    if (existing.status !== "active") {
      throw new ApiException("CONFLICT", `Cannot suspend key in status=${existing.status}`, 409);
    }
    ...
    await supabase.from("audit_logs").insert({
      ...
      action: env === "production" ? "api.production_key.suspended" : "api.sandbox_key.suspended",
      entity_type: "api_key",
      entity_id: key_id,
      metadata: { environment: env, api_client_id: existing.api_client_id, reason, request_id: requestId },
    });

    return new Response(JSON.stringify({ ok: true, key_id, status: "suspended" }),
      { status: 200, headers: { "Content-Type": "application/json", ...headers } });
  } catch (error) {
