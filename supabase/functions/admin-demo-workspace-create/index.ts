// OPS-010 — Admin: create demo workspace (platform_admin + AAL2 + reason >= 20)
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, { adminClient: admin, callerUserId: userData.user.id, action: "admin-demo-workspace-create" });
  } catch (e) {
  ...
  const { data, error } = await admin.rpc("create_demo_workspace", {
    ...
    p_org_name: parsed.data.org_name ?? null,
