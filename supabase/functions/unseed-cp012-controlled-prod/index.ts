/**
 * unseed-cp012-controlled-prod — Controlled production cleanup for the
 * single CP-012 Daniel acceptance fixture.
 *
 * Hard-gates every delete on:
 *   - hash IN ALLOWED_FIXTURE_HASHES, AND
 *   - is_demo = true, AND
 *   - metadata.fixture_scope = "CP-012 Daniel UAT".
 *
 * Does NOT touch auth users, profiles, orgs (reused across the Daniel
 * fixture system). poi_engagements + disputes rows cascade via existing
 * FKs on match_id.
 */
...
Deno.serve(async (req) => {
  ...
  if (body.confirm !== "RUN_UNSEED_CP012_CONTROLLED_PROD") {
      ...
      return json(
        {
          ...
          message:
            `admin_settings.${ADMIN_FLAG_KEY}.enabled must be true to clean CP-012 fixtures in production.`,
        },
        403,
      );
    }
  ...
  try {
    ...
    try {
      await admin.from("admin_audit_logs").insert({
        ...
        target_id: null,
        details: {
          function: "unseed-cp012-controlled-prod",
      ...
      console.error("[unseed-cp012-controlled-prod] audit insert failed:", e);
    }
    ...
    return json({
      ...
      notes: [
        ...
        "poi_engagements + disputes cascade via FK on match_id; no auth users / orgs / profiles removed.",
      ],
    });
  } catch (err) {
    return json(
