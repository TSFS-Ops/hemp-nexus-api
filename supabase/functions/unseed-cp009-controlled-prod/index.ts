/**
 * unseed-cp009-controlled-prod — Controlled production cleanup for the
 * single CP-009 / DEC-003 Daniel acceptance fixture.
 *
 * Hard-gates every delete on:
 *   - hash IN ALLOWED_FIXTURE_HASHES, AND
 *   - is_demo = true, AND
 *   - metadata.fixture_scope = "CP-009 / DEC-003 Daniel UAT".
 *
...
Deno.serve(async (req) => {
  ...
  if (body.confirm !== "RUN_UNSEED_CP009_CONTROLLED_PROD") {
      ...
      return json(
        {
          ...
          message:
            `admin_settings.${ADMIN_FLAG_KEY}.enabled must be true to clean CP-009 fixtures in production.`,
        },
        403,
      );
    }
  ...
  try {
    ...
    try {
      await admin.from("admin_audit_logs").insert({
        ...
        target_id: null,
        details: {
          function: "unseed-cp009-controlled-prod",
      ...
      console.error("[unseed-cp009-controlled-prod] audit insert failed:", e);
    }
