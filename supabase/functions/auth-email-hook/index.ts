...
const ROOT_DOMAIN = "izenzo.co.za"
const FROM_DOMAIN = "izenzo.co.za" // Domain shown in From address (may be root or sender subdomain)
...
const SAMPLE_PROJECT_URL = "https://compliance-matching.lovable.app"
...
async function handlePreview(req: Request): Promise<Response> {
  ...
  try {
    ...
    return wrap(new Response(JSON.stringify({ error: 'Invalid JSON in request body' }), {
      status: 400,
      headers: { ...previewCorsHeaders, 'Content-Type': 'application/json' },
    }))
  }
  ...
  if (!EmailTemplate) {
    return wrap(new Response(JSON.stringify({ error: `Unknown email type: ${type}` }), {
      status: 400,
      headers: { ...previewCorsHeaders, 'Content-Type': 'application/json' },
    }))
  }
