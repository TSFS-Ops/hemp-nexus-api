...
export async function assertWadSealIdvGate(
  ...
  for (const [side, orgId] of [
    ...
    if (!orgId) continue;
    // Look up any p5scr_subjects row linked to this org. Schema may vary;
    // we only require an org linkage column named `org_id`. If the table
    // or column is absent (or no row exists), we skip — that's the
    // Batch V-Wire boundary.
