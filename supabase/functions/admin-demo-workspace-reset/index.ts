// OPS-010 — Admin: reset demo workspace (platform_admin + AAL2 + reason >= 20)
...
Deno.serve(async (req) => {
  ...
  try {
    await assertAal2(authHeader, { adminClient: admin, callerUserId: userData.user.id, action: "admin-demo-workspace-reset" });
  } catch (e) {
  ...
  const { data, error } = await admin.rpc("reset_demo_workspace", {
