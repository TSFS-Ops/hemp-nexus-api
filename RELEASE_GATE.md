...
- `check-no-deprecated-compliance-provider-names.mjs` — Deprecated-compliance-provider cleanup guard: fails the build if any client-facing or edge-runtime source file names one of the deprecated integrations (CIPC / Onfido / Dow Jones / Refinitiv). Allowlists the SSOT back-compat alias maps (`src/lib/stub-providers.ts`, `supabase/functions/_shared/stub-providers.ts`, evidence-rating mirrors, `src/lib/idv/provider-registry.ts`), existing tests, and documentation. Pairs with the neutral provider vocabulary (`company registry provider`, `identity-document provider`, `sanctions/PEP screening provider`).
...
- Demo guard (`supabase/functions/_shared/demo-mode-guard.ts`) + entry helper (`_shared/demo-mode-entry.ts`) wired into:
  - Primary chokepoints: `send-transactional-email` (zero outbound), `token-purchase` (no live Paystack).
...
- `registry-bank-detail-start` — Batch 13 draft creation; gates on verified-email user + active scoped authority (`bank_detail_submission` / `bank_detail_update` / `bank_detail_revocation_request`); emits `registry_bank_detail_started`.
- `registry-bank-detail-evidence-upload` — Batch 13 evidence metadata capture (submitter or admin); emits `registry_bank_detail_evidence_uploaded` / `registry_bank_detail_evidence_metadata_added`.
- `registry-bank-detail-review` — Batch 13 admin/compliance review writer; every action except `assign_reviewer` requires a reason and `accept_captured_unverified` requires the pinned acknowledgement; emits `registry_bank_detail_review_started`, `registry_bank_detail_more_evidence_requested`, `registry_bank_detail_evidence_reviewed`, `registry_bank_detail_captured_unverified`, `registry_bank_detail_rejected`, `registry_bank_detail_disputed`, `registry_bank_detail_revoked`, `registry_bank_detail_expired`, `registry_bank_detail_superseded`.
- `registry-bank-detail-risk-evaluate` — Batch 13 admin/compliance risk-flag engine (holder mismatch, individual-holder-for-company, third-party, country mismatch, duplicate fingerprint, missing evidence); writes `registry_bank_detail_risk_flags` and emits `registry_bank_detail_risk_flag_added` / `registry_bank_detail_duplicate_fingerprint_detected`.
- `registry-bank-detail-revocation-request` — Batch 13 user-driven revocation; requires `bank_detail_revocation_request` scope; emits `registry_bank_detail_revocation_requested`.
- `registry-bank-detail-notification-log` — Batch 13 LOG-ONLY in-app notification ledger (`delivered_externally = false`); emits `registry_bank_detail_notification_logged`. No external send is wired.
- `registry-bank-detail-unmask-access` — Batch 13 elevated reasoned unmask (platform_admin / compliance_owner only, reason ≥20 chars); writes both `registry_bank_detail_unmask_access_logs` and `registry_bank_detail_access_log`; emits `registry_bank_detail_unmask_viewed`.
