...
- `check-data-004-phase3-audit-names.mjs` — DATA-004 Phase 3 audit-name SSOT: pins `data.retention_job.email_send_log.{started,completed,partial,failed,skipped}` to `purge-email-send-log-daily/index.ts`. Any vocabulary change for the wired sweeper must be paired here.
