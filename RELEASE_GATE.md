...
- `check-data-004-phase2-no-enforcement.mjs` — DATA-004 Phase 2 non-enforcement guard: scans `storage-retention-cleanup`, `account-deletion-sweeper`, `cold-storage-archive`, `email-log-anonymise`, and any `purge-email-send-log*` edge function for references to `org_retention_policies` or `get_effective_retention_days`. Phase 2 is observational/evidence only — no sweeper may yet consume the per-org retention table. Phase 3 will relax this guard one sweeper at a time, with explicit sign-off and per-job tests.
